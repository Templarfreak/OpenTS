...
/// or gives up on it.
/// </summary>
/// <param name="callback">Called between the steps of generating a map, and not while the
/// screen waits for the player.</param>
/// <returns>1 if the player accepted the map, otherwise 2.</returns>
...
/// <summary>
/// Clamps every map generation setting to its legal range. Call it after anything changes the
/// settings: reading the screen, a randomize, or a load. A tournament territory narrows the
/// ranges further; without the Firestorm addon, the mutated biome becomes temperate and the
/// other Firestorm settings are cleared.
/// </summary>
...
ScenarioState MapGeneratorClass::Generate_Random_Map(bool full_init, CCINIClass * scenario)
{

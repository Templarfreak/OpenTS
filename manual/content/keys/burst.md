...
The firing object keeps a running count of where it stands in the burst. Every shot but the last is followed by a short gap. The last shot of the burst is the one that pays [`ROF`](/keys/rof/#scope-weapontype), raised by a random zero to two frames and shortened by the veteran `ROF` ability. The scale applied to `ROF` is the house's rate-of-fire bias: the difficulty's [`ROF=`](/keys/rof/#scope-difficulty-settings) setting, multiplied by the house's own [`ROF=`](/keys/rof/#scope-housetype) outside campaigns. The `ROF` ability is granted by listing it in the object type's [`VeteranAbilities=`](/keys/veteranabilities/), or in [`EliteAbilities=`](/keys/eliteabilities/) as well at elite rank. The count then returns to the start. A structure with more than one round in stock ignores all of this: every shot leaves it rearming for a single frame, whatever the weapon's `Burst`, `BurstDelay` and `ROF` say. An [`Ammo=`](/keys/ammo/) left unset does not qualify, and the normal delays apply.
...
A weapon that draws a wave or a particle system, whether through [`IsSonic`](/keys/issonic/), [`IsRailgun`](/keys/israilgun/), [`UseFireParticles`](/keys/usefireparticles/) or [`UseSparkParticles`](/keys/usesparkparticles/), takes `ROF` between every shot and never the short gap. A burst on such a weapon therefore changes only the muzzle it alternates to.

:::caution[A building's first upgrade decides whether both slots fire]
Once a structure has anything plugged into its first upgrade slot, it stops choosing between its two weapons. It fires both slots in the same frame when the plugged-in type counts as a two shooter: that type's own weapons name the same weapon in both slots, or either of them sets a burst above one. When the plugged-in type does not count as a two shooter, the structure fires neither slot at all. Only the first upgrade slot is ever read, and the slots that fire or stay silent are the structure's own two weapon slots.
:::

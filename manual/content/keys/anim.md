...
The list length decides how it is spent. A length of `8`, `16`, `32` or `64` makes the choice directional, one entry per facing; any other non-empty length uses the first entry for every shot and ignores the rest. The animation is created at the muzzle coordinate the shot left from. On anything other than a building it is attached to the firing object, so that it travels with it.

...
when_omitted:
  ...
  note: "`1` when `Control=` sets `DECAY`, else `0`."
---

How many of the last names in [`Sounds=`](/keys/sounds/) are decay samples. A play draws one of them at random and plays it once after the body. A loop plays it after its last cycle, or after the current cycle finishes when the game ends the loop early. Writing the key sets the count outright, and `DECAY` in [`Control=`](/keys/control/) without it means one.

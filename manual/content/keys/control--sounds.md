...
- `PREDELAY`: the `Delay=` silence comes once, before the first sample, on one-shots as well as loops.
- `INTERRUPT`: when [`Limit=`](/keys/limit/) is reached by a copy as loud as the new one, the quietest gives way instead of the new one being refused.

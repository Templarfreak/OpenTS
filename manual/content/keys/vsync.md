...
The game presents no more often than the display refreshes, whatever this setting says. It measures the refresh rate and skips a present that would arrive early, leaving the frame marked so the next one shows the newest picture. Switching this on also makes each present *wait* for the display, which removes the torn seam that can appear across a scrolling map.
...
The setting is read before the renderer starts and applied then, so a change takes effect the next time the game is launched.

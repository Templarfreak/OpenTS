...
breaking: true
migration:
- Set `AutoScroll=yes` under `[Options]` in `sun.ini` where the file already sets `AutoScroll=no`, which had no effect before and now stops edge scrolling.
targets:
- type: key
  id: AutoScroll
  effect: changed
...
The game controls dialog has that switch as an Edge Scrolling check box, beside Scroll
